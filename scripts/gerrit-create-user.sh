...
ret=$(curl --output /dev/null --silent --write-out "%{http_code}" "http://localhost:8080/accounts/${username}")
if [[ ${ret} -eq 200 ]] ; then
...
do
  case "${type}" in
    ldap)
      ret=$(curl --request POST --data "username=${username}&password=${password}" --output /dev/null --silent --write-out "%{http_code}" http://localhost:8080/login)
      if [[ ${ret} -eq 302 ]]; then
    ...
    internal)
      ...
      ret=$(curl --request PUT --user "${admin_user}:${admin_password}" --header 'Content-Type: application/json; charset=UTF-8' --data "${json_request}" --output /dev/null --silent --write-out "%{http_code}" http://localhost:8080/a/accounts/"${username}")
      if [[ ${ret} -eq 201 ]]; then
